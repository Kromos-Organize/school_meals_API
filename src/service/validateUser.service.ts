...
export class ValidateUserService {
    ...
    async checkUser(email:string): Promise<UserDto> {
        ...
        if (manager) return {id: manager.manager_id, role_id: manager.role_id, email: manager.email, password: manager.password};
        if (employee) return {id: employee.employee_id, role_id: employee.role_id, email: employee.email, password: employee.password};
    }
}